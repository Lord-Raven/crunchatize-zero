...
const DEFAULT_OUTCOME_TEMPLATE = '<div style="--failure: 255, 0, 0; --mixed: 255, 140, 0; --success: 60, 179, 113; --critical: 176, 224, 230; margin: 0.5rem; ' +
    'border: 1px solid rgb(var(--{{total_class}})); border-radius: 8px; padding: 0.75rem 1rem; background: linear-gradient(to right, rgba(var(--{{total_class}}), 0.2), #0003); box-shadow: 0 2px 4px #0003; ' +
    'font-family: system-ui, sans-serif; text-shadow: 2px 2px 4px #0003; font-size: 2rem; line-height: 2rem;">' +
    '<div style="display: flex; justify-content: center; color: #bbb; font-size: 1rem;"><b><i>{{content}}</b></i></div>' +
    ...
    '<span style="color: rgb(var(--{{modifier_class}}));"> {{modifier_sign}} {{modifier_absolute}} <sup style="font-size: 1rem;">(difficulty)</sup></span>' +
